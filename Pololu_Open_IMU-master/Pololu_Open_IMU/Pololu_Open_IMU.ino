...
void loop() {
	...
	if (millis() - displayTimer > 250) {
		displayTimer = millis();
		GetEuler();
    String gyro_string = String("!ANG:: " + String((int)rollInDegrees, DEC));   // Formatting output msg (There must be a better way to do this)
    gyro_string = String(gyro_string + " , ");
    gyro_string = String(gyro_string + String((int)pitchInDegrees, DEC));
    gyro_string = String(gyro_string + " , ");
    gyro_string = String(gyro_string + String((int)yawInDegrees, DEC));
